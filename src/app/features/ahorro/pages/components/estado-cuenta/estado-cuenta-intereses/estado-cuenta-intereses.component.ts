...
export class EstadoCuentaInteresesComponent implements OnInit, OnDestroy {
  ...
  generarAnios(): void {
    const anioActual = Math.max(this.fechaServidor.getFullYear(), 2026);
    this.anios = [];

    for (let y = anioActual; y >= 2026; y--) {
      this.anios.push(y);
    }
  ...
  seleccionarConsulta(tipo: TipoConsultaInteres): void {
    ...
    if (tipo === 'Trimestre') this.setFechasPorTrimestre();
    if (tipo === 'Mes') this.setFechasPorMes();
    if (tipo === 'Anio') this.setFechasPorAnio();
  ...
  aplicarFiltros(): void {
    ...
    if (this.tipoConsulta === 'Trimestre') this.setFechasPorTrimestre();
    if (this.tipoConsulta === 'Mes') this.setFechasPorMes();
    if (this.tipoConsulta === 'Anio') this.setFechasPorAnio();
  ...
  setFechasPorAnio(): void {
    this.fechaDesde = `${this.anio}-01-01`;
    this.fechaHasta = `${this.anio}-12-31`;
  }
